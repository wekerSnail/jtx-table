    ...
    */
    ...
    _revertData: function _revertData(rows, prop) {
      ...
      if (arguments.length) {
        ...
        rows.forEach(function (row) {
          ...
          if (oRow && row) {
            if (prop) {
              _xeUtils.default.set(row, prop, _xeUtils.default.clone(_xeUtils.default.get(oRow, prop), true));
            } else {
    ...
    _setActiveCell: function _setActiveCell(row, prop) {
      ...
      return this.scrollToRow(row, true).then(function () {
        if (row && prop) {
          var column = _xeUtils.default.find(_this7.visibleColumn, function (column) {
            return column.property === prop;
          });

          if (column && column.editRender) {
            var cell = _tools.DomTools.getCell(_this7, {
              row: row,
              column: column
            });
