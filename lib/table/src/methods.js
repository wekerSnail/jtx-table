...
var Methods = {
  ...
  reloadRow: function reloadRow(row, record, prop) {
    ...
    if (oRow && row) {
      if (prop) {
        _xeUtils.default.set(oRow, prop, _xeUtils.default.get(record || row, prop));
      } else {
        if (record) {
    ...
    if (oRow && row) {
      if (prop) {
        ...
        var keys = Object.keys(row).filter(function (key) {
          return _xeUtils.default.has(oRow, key);
        });
  ...
  */
  setColToEvent: function setColToEvent(oRow, row, keys) {
    ...
    columns.forEach(function (column) {
      ...
      var value = _xeUtils.default.get(row, column.property);

      oRow[property] = value;

      if (_xeUtils.default.has(editRender, 'events')) {
  ...
  isUpdateByRow: function isUpdateByRow(row, prop) {
    ...
    if (oRow) {
      if (arguments.length > 1) {
        return !_xeUtils.default.isEqual(_xeUtils.default.get(oRow, prop), _xeUtils.default.get(row, prop));
      }
  ...
  handleTooltip: function handleTooltip(evnt, column, row) {
    ...
    if (this.tooltipOpts.toHtml) {
      content = cell.firstChild.firstChild ? _tools.UtilTools.toHtml(cell.firstChild.firstChild) : cell.innerText;
    } else {
    ...
    if (content && wrapperElem.scrollWidth > wrapperElem.clientWidth || column.showTooltipAll) {
      ...
      if (tooltip) {
        tooltip.toVisible(cell, this.tooltipOpts.toHtml ? content : _tools.UtilTools.formatText(content));
      }
    }
